...
template<bool inv> class F {
private :
    int rounds = 0;
    std::vector<int> e = {31,0,1,2,3,4,3,4,5,6,7,8,7,8,9,10,11,12,11,12,13,14,15,16,15,16,17,18,19,20,19,20,21,22,23,24,23,24,25,26,27,28,27,28,29,30,31,0};
    std::vector<int> p = {15,6,19,20,28,11,27,16,0,14,22,25,4,17,30,9,1,7,23,13,31,26,2,8,18,12,29,5,21,10,3,24};
    ...
    int sbox4[64] = {7,13,14,3,0,6,9,10,1,2,8,5,11,12,4,15,13,8,11,5,6,15,0,3,4,7,2,12,1,10,14,9,10,6,9,0,12,11,7,13,15,1,3,14,5,2,8,4,3,15,0,6,10,1,13,8,9,4,5,11,12,7,2,14};
    int sbox5[64] = {2,12,4,1,7,10,11,6,8,5,3,15,13,0,14,9,14,11,2,12,4,7,13,1,5,0,15,10,3,9,8,6,4,2,1,11,10,13,7,8,15,9,12,5,6,3,0,14,11,8,12,7,1,14,2,13,6,15,0,9,10,4,5,3};
    ...
    int* arg[8] = {sbox1, sbox2, sbox3, sbox4, sbox5, sbox6, sbox7, sbox8};
    KeyGen<inv> keygen;
    S_Fonction s_fonction;

public :
    F(SequenceD<64> seq) {
        keygen = KeyGen<inv>(seq);
        s_fonction = S_Fonction(arg);
    }

    Sequence operator()(Sequence s) {
        if(rounds < 16) {
            Sequence expanded = s.permutation(e);
            SequenceD<48> key = keygen.next();
            Sequence xored = expanded*Sequence({key.left(), key.right()});
            Sequence sboxed = s_fonction(xored);
            Sequence permuted = sboxed.permutation(p);
            rounds++;
            return permuted;
        }
    }
